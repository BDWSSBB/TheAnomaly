...
public class EveryCardIsDecoyToYouAction extends AbstractGameAction {

    private AbstractPlayer player;

    public EveryCardIsDecoyToYouAction() {
    ...
    public void update() {
        ...
        if (cardsDiscarded > 0) {
            ...
            AbstractDungeon.actionManager.addToTop(new GainEnergyAction(cardsDiscarded));
        }
        for (AbstractCard c : this.player.hand.group) {
            if (c.type == AbstractCard.CardType.STATUS || c.type == AbstractCard.CardType.CURSE || (c instanceof AbstractAnomalyCard && ((AbstractAnomalyCard) c).baseImprobabilityNumber > 0)) {
